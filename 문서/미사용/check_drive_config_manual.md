...
```powershell
powershell -ExecutionPolicy Bypass -File "d:\00.개발\03.new_valuation\check_drive_config.ps1"
```

### 서비스 계정 연결 확인

Drive 업로드가 동작하려면 아래 3가지를 먼저 맞춰야 합니다.

1. Google Cloud에서 Drive API가 활성화되어 있어야 합니다.
2. JSON 키 파일이 실제로 존재해야 합니다.
3. Google Drive 대상 폴더에 서비스 계정 이메일을 `편집자`로 공유해야 합니다.
...
```powershell
$configPath = "d:\00.개발\03.new_valuation\up_valuation_config.json"
...
```powershell
# 실제 파일 경로 확인
Get-Item "d:\00.개발\03.new_valuation\keys\*.json"

# 파일 목록 출력
ls "d:\00.개발\03.new_valuation\keys\"
```
...
```powershell
# 1. 설정 파일 확인
cat "d:\00.개발\03.new_valuation\up_valuation_config.json"

# 2. enableGoogleDriveUpload가 true인지 확인
# 3. 경로에 오류(따옴표, 기호)가 없는지 확인
# 4. JSON 파일 권한 확인 (읽기 가능)
Test-Path "d:\00.개발\03.new_valuation\keys\service_account.json" -PathType Leaf
...
- [ ] 서비스 계정 생성 완료
- [ ] JSON 키 파일 다운로드 완료
- [ ] JSON 파일을 `d:\00.개발\03.new_valuation\keys\` 에 저장 완료
