...
def _latest_reports() -> list[Path]:
    ...
    for path in OUTPUT_DIR.glob("상세리포트_*.csv"):
        if not path.is_file():
            continue
...
def _build_trade_rows(reports: pd.DataFrame, price: pd.DataFrame, horizons: list[int], ks_side_bps: float, kq_side_bps: float) -> pd.DataFrame:
    ...
    for _, row in reports.iterrows():
        ticker = str(row.get("종목코드", "") or "").zfill(6)
        ...
        combined_action = str(row.get("결합액션", "") or "").strip()
        signal_score = pd.to_numeric(row.get("종합점수", np.nan), errors="coerce")
        signal_weight = pd.to_numeric(row.get("권장비중(%)", np.nan), errors="coerce")
        market = str(row.get("시장", "") or "").strip()
        cost_bps = _cost_bps_for_market(market, ks_side_bps, kq_side_bps)
        is_signal = combined_action != "제외"
