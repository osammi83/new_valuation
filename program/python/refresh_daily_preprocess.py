...
def parse_number(value: object) -> float:
    if value is None:
        return np.nan
    cleaned = str(value).replace(",", "").replace("??, "").replace("諛?, "").strip()
    if cleaned in {"", "-", "N/A"}:
        return np.nan
    try:
...
def maybe_rebuild_universe_csv(top_n_kospi: int = 400, top_n_kosdaq: int = 400) -> Optional[pd.DataFrame]:
    ...
    pat = re.compile(r"(??|\(??)|?ㅽ뙥|SPAC|ETF|ETN)", re.IGNORECASE)
...
def _dart_fetch_eps_by_year(api_key: str, stock_code: str, year: int) -> float:
    ...
    for it in items:
        ...
        if not account_nm:
            continue
        if ("二쇰떦" in account_nm and "?댁씡" in account_nm) or ("EPS" in account_nm.upper()):
