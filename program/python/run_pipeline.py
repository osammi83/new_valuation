...
def main() -> int:
    ...
    if args.command == "backtest":
        backtest_command = [
            sys.executable,
            str(SCRIPT_DIR / "backtest_walkforward.py"),
            "--horizons",
            str(args.horizons),
            "--thresholds",
            str(args.thresholds),
    ...
    try:
        ...
        _run_step(
            ...
            [python_exe, str(SCRIPT_DIR / "preprocess_daily_updates.py"), "--days-back", str(daily_days_back)],
        )
        _run_step(
            "Daily 2/7 refresh_assumptions.py",
            [python_exe, str(SCRIPT_DIR / "refresh_assumptions.py")],
        )

        market_command = [python_exe, str(SCRIPT_DIR / "ingest_market_data.py"), "--period", daily_period]
        if daily_limit is not None:
            market_command.extend(["--limit", str(daily_limit)])
        _run_step("Daily 3/7 ingest_market_data.py", market_command)

        feature_command = [python_exe, str(SCRIPT_DIR / "build_features_daily.py"), "--period", daily_period]
        if daily_limit is not None:
            feature_command.extend(["--limit", str(daily_limit)])
        _run_step("Daily 4/7 build_features_daily.py", feature_command)

        signal_command = [python_exe, str(SCRIPT_DIR / "score_daily_signals.py")]
        _run_step("Daily 5/7 score_daily_signals.py", signal_command)

        _run_step("Daily 6/7 manage_positions.py", [python_exe, str(SCRIPT_DIR / "manage_positions.py")])

        build_command = [python_exe, str(SCRIPT_DIR / "build_daily_report.py"), "--period", daily_period, "--output-mode", daily_output_mode]
        if daily_limit is not None:
        ...
        _run_step(
            ...
            [python_exe, str(SCRIPT_DIR / "validate_outputs.py")] + (["--allow-empty-core-selection"] if daily_limit is not None else []),
        )

        _run_step(
            "Validation 2/2 validate_csv_contracts.py",
            [python_exe, str(SCRIPT_DIR / "validate_csv_contracts.py")],
        )
        ...
        if drive_enabled or (folder_id and service_account):
            ...
            drive_result = subprocess.run([python_exe, str(SCRIPT_DIR / "publish_to_drive.py")], cwd=BASE_DIR, check=False)
