...
& $pythonRunner.Exe @($pythonRunner.Args) (Join-Path $projectRoot 'program\python\run_pipeline.py') daily @Args
if($LASTEXITCODE -ne 0){ throw '[ERROR] run_pipeline.py failed' }

if($githubAutoSyncEnabled){
	Write-Host '[Info] Syncing changes to GitHub...'
	& (Join-Path $projectRoot 'program\powershell\sync_github.ps1') -RemoteName $githubRemoteName -Branch $githubBranch -CommitPrefix $githubCommitPrefix
